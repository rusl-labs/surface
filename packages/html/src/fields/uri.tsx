...
/**
 * First-class URI view for JSON Schema formats `uri`, `uri-reference`,
 * `iri`, `iri-reference`, and annotation `widget.name: "uri"`.
 * Input: `<input type="url">`. Display: `<a href>`.
 */

type UriFieldProps = WidgetProps<string> & {
  readonly minLength?: number | undefined;
  readonly maxLength?: number | undefined;
  readonly pattern?: string | undefined;
};

/** Presentational URI display. No `useSurface`. */
export function UriValue({ value }: WidgetProps<string>): ReactElement {
  const uri = typeof value === "string" ? value.trim() : "";
  if (uri.length === 0) {
    return <span className={surfaceClass.value}>—</span>;
  }
  return (
    <a className={surfaceClass.uri} href={uri}>
      {uri}
    </a>
  );
}

/** Presentational URL control. No `useSurface`. */
...
export function UriDisplay(_props: SurfaceProps): ReactElement {
  const { data, id, entry } = useSurface();
  return (
    <FieldChrome as="div" className={surfaceClass.uriHost}>
      <UriValue
...
export function UriInput(_props: SurfaceProps): ReactElement {
